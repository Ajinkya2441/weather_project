...
# Caching dictionary for previously fetched weather data
weather_cache = {}
...
async def fetch_weather(session, lat, lon):
    ...
    if cache_key in weather_cache:
        return weather_cache[cache_key]  # Return cached data
    ...
    try:
        async with session.get(url) as response:
            ...
            print(data)  # Debugging line to check the API response
            weather_cache[cache_key] = data  # Cache the fetched data
            return data
...
async def get_weather(lat, lon):
    ...
    try:
        ...
        humidity = data["main"]["humidity"]  # Ensure this is correct
        wind_speed = data["wind"]["speed"]  # Wind speed in m/s
        
        # Convert wind speed to km/h
        wind_speed_kmh = wind_speed * 3.6
        
        # Update the result label with the correct values
        result_label.config(text=f"🌡 Temperature: {temp}°C\n"
                               f"☁ Condition: {weather_desc}\n"
                               f"💧 Humidity: {humidity}%\n"
                               f"🌬 Wind Speed: {wind_speed_kmh:.2f} km/h", fg="white")
...
async def show_pressure(lat, lon):
    ...
    try:
        pressure = data["main"]["pressure"]  # Ensure this is correct
        humidity = data["main"]["humidity"]  # Ensure this is correct
        wind_speed = data["wind"]["speed"]  # Wind speed in m/s
